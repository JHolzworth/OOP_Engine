...
public class MixLevel extends Level{
	...
	private static Collection<Enemy> getEnemies(){
		...
		for(int i=0;i<10;i++)
			enemies.add(new ObtainableEnemy(new Point((int)(Math.random()*900)+50,(int)(Math.random()*700)+200), 25, 1));
		for(int i=0;i<5;i++)
			enemies.add(new FlickeringEnemy(new Point((int)(Math.random()*100)+50,(int)(Math.random()*700)+200), 25, 1));
		return enemies;
	}
